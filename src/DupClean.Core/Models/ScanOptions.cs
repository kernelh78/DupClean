...
public sealed class ScanOptions
{
    ...
    /// <summary>이 크기 이하 파일은 전체 해시 직접 계산 (Sparse 생략). 기본 32MB.</summary>
    public long DirectHashThresholdBytes { get; init; } = 32L * 1024 * 1024;

    /// <summary>pHash 유사 중복 탐지 활성화 여부.</summary>
    public bool ComputePHash { get; init; } = true;

    /// <summary>pHash Hamming Distance 임계값. 이하이면 유사 이미지로 판정. 기본 8.</summary>
    public int PHashThreshold { get; init; } = 8;
}
